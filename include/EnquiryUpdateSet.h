...
#if !(defined __6dEnquiryUpdateSet_H)
#define __6dEnquiryUpdateSet_H
...
/************************************************************************
 * Class : EnquiryUpdateSet
 * Purpose : to perform updation in enquiry related details in InstanceMaster
 ************************************************************************/

class CEnquiryUpdateSet: public CMySQLRecordSet
{
	private:
		/**
		 * Private Variables
	...
	public:
		...
		void mdfn_getSQLString(char *pscL_Str)
		{
			...
			sprintf(pscL_Str, "UPDATE %s SET LAST_HEARTBEAT_TIME = current_timestamp(),HEARTBEAT_MISS_COUNT=0 WHERE SIGNALLING_PORT=%ld AND SIGNALLING_IP='%s'",CG_AppConfigParams.mcfn_getInstanceMasterTable(),mesl_SignalingPort,pmesc_SignalingIp);
			EVT_LOG(CG_EventLog, LOG_INFO | LOG_OPINFO,siG_InstanceID, "getSQLString","MySqlQuery", this,"", "%s",pscL_Str);

	}

		bool 	mcfn_updateEnquiryTime(const char*,const long&);
